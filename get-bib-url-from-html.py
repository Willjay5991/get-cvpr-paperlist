...
def extract_paper_info(html_content):
    """
    从HTML内容中提取论文的BibTeX引用和URL链接
    
    参数:
        html_content: HTML字符串内容
        
    返回:
        包含论文信息的列表，每个元素是一个字典，包含'title', 'url', 'bibtex'键
    """
...
if __name__ == "__main__":
    ...
    # 读取HTML文件
    with open('CVPR 2025 Open Access Repository.html', 'r', encoding='utf-8') as html_file:
        html_content = html_file.read()
        print(f'have read html file')
    
    outfile = 'cvpr-papers.bib'
    papers = extract_paper_info(html_content)
    wirte_to_file(papers, outfile)
    print("have writen paper infor into {}".format(outfile))

    # convert bibtex to ris
    from convert2risViare import *

    # 解析BibTeX
    entries = parse_bibtex(papers)
    ...
    with open("references.ris", "w", encoding="utf-8") as f:
        f.write(ris_output)
    print(f"have convert {num} bib entries to cvpr-papers.ris")
